...
def setup_plot():
    ...
    if current_mode == INTENSITY_MODE:
        ...
        if current_mode == POSITION_MODE:
            ...
            ax1.plot(time_data, x_pos_data)
            ax2.plot(time_data, y_pos_data)
        elif current_mode == POWER_MODE:
            ax1.set_ylabel('Power AB')
            ax2.set_ylabel('Power CD')
            ax1.plot(time_data, power_a_data, 'b-', label='A')
            ax1.plot(time_data, power_b_data, 'r-', label='B')
            ax2.plot(time_data, power_c_data, 'b-', label='C')
            ax2.plot(time_data, power_d_data, 'r-', label='D')
            ax1.legend()
            ax2.legend()
        elif current_mode == RMS_MODE:
            ax1.set_ylabel('X res. rms (um)')
            ax2.set_ylabel('Y res. rms (um)')
            ax1.plot(time_data, x_rms_data)
            ax2.plot(time_data, y_rms_data)
...
def process_data(host, port, action_queue, output_lock):
    # -------------------------------------------------------------------------------
    # Initial Parameters
    BPM_DIA = 40
...
if __name__ == '__main__':
    ...
    action_queue = Queue()
    p = Process(target=process_data, args=(host, port, action_queue, output_lock))
    p.start()
